...
test('POS settings carry a provider-neutral customer email connector', () => {
  ...
  assert.match(sharedTypes, /rewards_redeemed: Array<\{/)
  assert.match(sharedTypes, /reward_name: string/)
  assert.match(sharedTypes, /points_used: number/)
  assert.match(sharedTypes, /net_dollar_value_applied: number/)
...
test('customer email connector builds and invokes a standard receipt payload', () => {
  ...
  assert.match(connector, /buildReceiptRewardRedemptions\(sale\)/)
  assert.match(connector, /rewards_redeemed: rewardsRedeemed\.map/)
  assert.match(connector, /Rewards redeemed/)
  assert.match(connector, /Points used/)
  assert.match(connector, /Net value applied/)
  assert.match(connector, /sale\.lines\.filter\(\(line\) => !isFranRewardReceiptLine\(line\)\)/)
