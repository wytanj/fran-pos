...
test('shared package exports SKUMS POS graph contracts', () => {
  ...
  assert.match(sharedTypes, /export interface SkumsPosBasketQuoteInput/)
  assert.match(sharedTypes, /export interface SkumsPosBasketQuoteLine extends SkumsGraphRefs/)
  assert.match(sharedTypes, /export interface SkumsPosReservationInput/)
...
test('sale adapter maps completed POS sales into SKUMS sale payloads', () => {
  ...
  assert.match(adapter, /getActiveStore\(\)/)
  assert.match(adapter, /getPosRegisterCode\(\)/)
  ...
  // fran-skums POST /api/v1/pos/sales expects location_id + register_id (not hard-coded STORE)
  assert.match(adapter, /location_id: store\.inventoryLocationId/)
  assert.match(adapter, /register_id: getPosRegisterCode\(\)/)
...
test('SKUMS client targets POS scan and sale API endpoints with account connector settings', () => {
  ...
  assert.match(client, /export async function quoteSkumsPosBasket/)
  assert.match(client, /export async function createSkumsPosReservation/)
  assert.match(client, /export async function commitSkumsPosReservation/)
  assert.match(client, /export async function releaseSkumsPosReservation/)
  ...
  assert.match(client, /\$\{config\.apiUrl\}\/fran\/pos\/basket\/quote/)
  assert.match(client, /\$\{config\.apiUrl\}\/fran\/pos\/reservation\/create/)
...
test('POS sale page can load SKUMS catalog while retaining mock fallback', () => {
  ...
  assert.match(salePage, /quoteSkumsPosBasket/)
